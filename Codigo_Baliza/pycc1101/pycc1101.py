...
class TICC1101(object):
    ...
    def setCarrierFrequency(self, freq=868):
    ...
    def sendData(self, dataBytes):
        ...
        while remaining_bytes != 0:
            ...
            if self.debug:
                #t0=t0+1
                #print(t0)
                #print("Waiting until all bytes are transmited, remaining bytes")
                #print(t0)
                #print(time.time())
    ...
    def recvData(self):
        ...
        if (rx_bytes_val & 0x7F and not (rx_bytes_val & 0x80)):
            ...
            data = self._readBurst(self.RXFIFO, 8)
            ...
            #print("Data: ") + str(data)
            print (data)
            file1=open("pru_rx1.txt","a")
            file1.write(str(data)+"\n")
            
            print("La temperatura es: ", data[2], ".", data[3])
            
            if(data[4] == 0):
                print("Agulo con el eje x: ", data[5])
            elif(data[4] == 1):
                print("Angulo con el eje x: -", data[5])
            
            if(data[6] == 0):
                print("Angulo con el eje y: ", data[7])
            elif(data[6] == 1):
                print("Angulo con el eje y: -", data[7])
            print("BPM:", data[8])
            self._flushRXFifo()
